...
class Statistics extends Component {
  ...
  getTopCustomers(users, orders){
    ...
    for (var order in orders){
        // console.log(uniqueIds.indexOf(orders[order].product))
        if (uniqueIds.indexOf(orders[order].buyerId) === -1){
            uniqueIds.push(orders[order].buyerId)
        }
    }
    ...
    for (var id in uniqueIds){
        ...
        console.log('hi ', users.filter(obj => { return obj.id === uniqueIds[id] }))

        dataStruct.push([users.filter(obj => { return obj.id === uniqueIds[id] })[0],
                        orders.filter((obj) => obj.buyerId === uniqueIds[id]).length])
        // console.log('yeah', orders.filter((obj) => obj.product === uniqueIds[id]).length)
        // dataStruct.push([products.filter(obj => { return obj.id === uniqueIds[id] })[0],
        //                 3])
            ...
            <div className="row">
                ...
                <Paper style={{width: '100%', marginTop:'3%'}}>
                    ...
                    {value => {
                        ...
                        const totalMonthly = this.getMonthlySpending(value.sellerOrders)
                        // console.log('totalSpendings', totalSpending)
                        return <Chart
                ...
                <Paper style={{width: '100%', marginTop:'3%'}}>
                    ...
                    {value => {
                        ...
                        const orderedProducts = this.orderProducts(value.products, value.sellerOrders)
                ...
                {value => {
                    ...
                    const orderedCustomers = value.userData.length === 0 
                        ? []
                        : this.getTopCustomers(value.userData, value.sellerOrders)
                    // console.log(orderedProducts)
                    return <List style={{width:'98%', marginBottom:'2%'}}>
